...
### s1.t4 — "Profile Saved"
> Five questions answered, your vibe locked in. `~/.pbx-lab/user-profile.json` exists now — tech level, comms style, goal, consent, autonomy. I'll talk to you the way you wanted from here on. Anytime you want to recalibrate, just say "run the personality quiz".
...
### s1.t6 — "Wallet Set Up"
> You picked fresh / import / defer, and the server quietly autogenerated `~/.pbx-bots/local.env` at mode 0600. `BOT_API_TOKEN`, `BOT_MASTER_KEY` (64-hex), and `BOT_HD_MNEMONIC` (24 words) — all in there. The wallet your fleet rides on now exists on disk.
...
### s1.t11 — "Cron Locked"
> All 6 `BEARWATCH-*` scheduled tasks are live — HealthCheck, WeatherPull, DailyDigest, StateBackup, CodebaseBackup, MetaWatchdog. `schtasks /query` shows them all `Ready`. The boring monitoring stuff runs itself now.
