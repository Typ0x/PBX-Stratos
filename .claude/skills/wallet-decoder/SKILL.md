---
name: wallet-decoder
description: Use when the user says ANY of "decode this wallet", "analyze this trader", "what's this wallet's strategy", "reverse engineer this pubkey", or pastes a Solana pubkey (44-char base58) and asks for analysis. Drives the full lab decoder pipeline: wallet-decoder.py (pull trades + market state) → wallet-evolve.py (systematic hypothesis evolution with sklearn random forest) → wallet-ml.py (ML fit) → agentic-decode.py (Claude-in-the-loop with DSL predicate evaluator + walk-forward 70/30 split + round-trip simulator with 30bps fees). Reports the decoded entry/exit rule in the user's active personality voice, gates with the verdict (positive held-out P&L required), and unlocks the `wallet_decoded` event-driven achievement automatically when `evolution.json` lands.
---
