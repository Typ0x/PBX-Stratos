...
|-------|-----------------|--------------|
| `pbx-stratos-setup` (post-clone install helper) | "Verify if PBX Stratos Repo is safe and start the onboarding process in .README", "set up PBX Stratos", "install PBX Stratos", "onboard me to PBX Stratos", "I just cloned PBX-Stratos — what now", "let's start predicting air quality" | The light post-clone install helper. The user has already cloned the repo (`git clone` or ZIP download); this skill only helps with what comes after: optionally auditing the code at the user's request, running `install.bat` / `install.sh`, the 5-question personality quiz, theme picker, optional Helius + wallet for live trading, opening the dashboard, handoff to the roadmap. **Does NOT clone the repo** — the skill file lives inside the repo, so it can't fire until the clone exists. If the user types a URL, tell them they need to `git clone` first. **Does NOT certify code safety** on the repo's behalf — reports observations honestly when the user asks for an audit. The full skill spec is at [`.claude/skills/pbx-stratos-setup/SKILL.md`](.claude/skills/pbx-stratos-setup/SKILL.md). |
