...
| Legacy install entry points | `PBX-Stratos/install.sh`, `setup.ps1` (root-level) | Pre-bootstrap path for users who want the old Python-only installer or Windows-specific setup. Both still work alongside the canonical `scripts/bootstrap.*` |
